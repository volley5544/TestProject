...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    testProjectAuthUserSubject.add(
      TestProjectAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<TestProjectAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  TestProjectAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = TestProjectAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    testProjectAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? UserDataModelStruct.fromSerializableMap(
...
TestProjectAuthUser? currentUser;
